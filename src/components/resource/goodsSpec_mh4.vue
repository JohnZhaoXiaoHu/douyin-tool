...
export default {
    ...
    methods: {  
        ...
        son_sconfirmCilck(tableData, obj, itemId){
            ...
            // let goodsMapKeys = [];
            // let indexs = {};
            // 所有的 已经选中的商品Id放入 数组中（father_goodsMapKeys）
            let hasKeys = false;
            for (let i = 0; i < this.father_tableData.length; i++) {
                const element = this.father_tableData[i];
                this.father_goodsMapKeys.push(element.itemId);
                console.log('---商品管理：子组件获取响应= '+i +'、element.id= '+element.itemId);
                this.father_indexs[element.itemId] = i;
            }
            if (this.father_goodsMapKeys.length>0) {
                hasKeys = true
            }
            console.log('---goodsSpec_mh4 上架商品4', this.tableData.length+ '、father_goodsMapKeys= '+this.father_goodsMapKeys); 
            if (this.tableData.length==0) {
                console.log('---商品列表 未请求完毕！！！'); return;
            }
            let self = this;
                ...
                if (hasKeys) {
                    ...
                    if (zm_beArrayContains1(item.id, self.father_goodsMapKeys)) {
                        ...
                        var spuIndex = self.father_indexs[item.id]; //找到已选的位置
                         const integralSPU    = self.father_tableData[spuIndex]; //拿出SPU 
                        for (let n = 0; n < item.skuList.length; n++) {
                            
                            const originSku = item.skuList[n];
                            const selectSku = integralSPU.skuList[n];
                            // 把已选中的商品的字段 复制给 原来的商品
                            originSku.integralId                = selectSku.integralId;
                            originSku.integralInventoryTotal    = selectSku.integralInventoryTotal;
                            originSku.integralInventorySurplus  = selectSku.integralInventorySurplus;
                            originSku.exchangeIntegral          = selectSku.exchangeIntegral;
                            originSku.exchangeMoney             = selectSku.exchangeMoney;
                            originSku.skuInventorySurplus       = selectSku.skuInventorySurplus;
                            
                            originSku.skuInventoryTotal         = selectSku.skuInventoryTotal;
                            originSku.skuId                     = selectSku.skuId;
                            originSku.status                    = selectSku.status;
                        }
                    }else{
                        item.numId = false; //未选中
                    }
        ...
        getChild () {
            ...
            this.$parent.fatherGoodsMapKeys = this.father_goodsMapKeys;
