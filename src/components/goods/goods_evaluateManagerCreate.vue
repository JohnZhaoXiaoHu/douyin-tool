                    ...
                    <div class="goods-card_body marketing">
                        ...
                        <div class="mh-coupon-divtop">
                            <el-breadcrumb separator-class="el-icon-arrow-right">
                                ...
                                <el-breadcrumb-item :to="{ path: '/goodslist/goods_evaluateManagerCreate' }">创建评价</el-breadcrumb-item>
                            </el-breadcrumb>   
                        </div>
                        <!-- 面包屑over -->
                        ...
                        <!-- <div class="zm_dl vip-alllist vip-xxxx">
                            ...
                            <div class="product_filesWall" id="product_filesWall">
                                ...
                                <el-dialog :visible.sync="dialogVisible1">
                                    <img width="100%" :src="emc_dialogImageUrl" alt="">
                                </el-dialog>
                            </div>
                        </div> -->
                        ...
                        <div class="emc_btnView">
                            <el-button class="emc_saveBtn1" type="primary" size="medium" @click="saveCilck(1)">继续评论该商品</el-button>
                            <el-button class="emc_saveBtn" type="primary" size="medium" @click="saveCilck(2)">保存并新建</el-button>
...
export default {
    ...
    mounted(){
        ...
        this.getData();
        var getRandomNum = Math.floor(Math.random()*10);  
        console.log('--------getRandomNum= '+getRandomNum);
        console.log('--------getRandomNum= '+Math.floor(Math.random()*10));
        console.log('--------getRandomNum= '+Math.floor(Math.random()*10));
        console.log('--------getRandomNum= '+Math.floor(Math.random()*10));
        console.log('--------getRandomNum= '+Math.floor(Math.random()*10));
    },
    ...
    methods:{
        ...
        getData(){
            //请求：机器人列表
            let self = this;
            let urlStr = baseapi.TIP + baseapi.qg_TYsetFindall;
            urlStr = 'http://172.16.1.84:8083/panicbuy/robot';
            urlStr = baseapi.TIP + '/shop/panicbuy/robot';
            console.log('--机器人列表 urlStr= ', urlStr);
            this.$http({
                method: "post",
                url: urlStr,
                'Content-Type':'Application/json'
            }).then(function(res){
                console.log('---机器人列表 response= '+zm_jsonToString(res.data.length));
                if(res.status ==200){
                    self.robotList = res.data;   
                // var getRandomNum = Math.floor(Math.random()*self.robotList.length);  
                // console.log('---getRandomNum= '+getRandomNum);
                // console.log('---getRandomNum= '+Math.floor(Math.random()*self.robotList.length));
                // console.log('---getRandomNum= '+Math.floor(Math.random()*self.robotList.length));
                // console.log('---getRandomNum= '+Math.floor(Math.random()*self.robotList.length));
                // console.log('---getRandomNum= '+Math.floor(Math.random()*self.robotList.length));
                } else {
        ...
        saveCilck(type){
            ...
            if(this.selectProduct.imageUrl=='' || this.selectProduct.imageUrl==null){
                this.zm_alterView('商品不能为空！', 'warning');
                return;
            }
            if(this.emc_content=='' || this.emc_content==null){
                this.zm_alterView('评论内容不能为空！', 'warning');
                return;
            }
            let params = new FormData();  
            // params.append('itemId',this.selectProduct.itemId);
            params.append('itemId',this.selectProduct.id);
            params.append('content',this.emc_content);
            params.append('no','')
            params.append('status',this.emc_type*1)
            ...
            // var getRandomNum = Math.floor(Math.random()*this.robotList.length); 
            // var robot =  this.robotList[getRandomNum];
            // console.log('---getRandomNum= '+getRandomNum);

            // spSpuid: this.selectProduct.itemId,
            // spSkuid: this.selectProduct.id,
            // let self = this;
            let urlStr = baseapi.TIP + baseapi.qg_createPanicbuy;
            urlStr = baseapi.TIP + "/order/evaluation/admin/add";
            // urlStr = "http://172.16.1.218:8080/evaluation/admin/add";
