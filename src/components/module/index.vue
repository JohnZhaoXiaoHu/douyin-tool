                     ...
                     <div class="container">
                      <div class="component-small">
                        <div class="component-small-list">
                          ...
                          <!-- <div class="wx-use-module wx-conmoent-btn" v-on:click="savePage" >保存</div>
                          <div class="wx-use-module wx-conmoent-btn" @click="closeCao">取消</div> -->
                        </div>
                      </div>
                      <div class="mh-module-style">
                         <div class="virtual-container" :style="{backgroundColor: pageCustomAttribute.backgroundColor, backgroundImage: 'url(' + pageCustomAttribute.backgroundImage + ')'}">
                          ...
                          <div v-on:click="componentClick" :data-no="item.no" class="component" :class="item.no == active.no ? 'active' : ''" v-for="(item, index) in components" :key="item.no">
                            ...
                            <customseven v-if="item.id == 12" :key="item.no" :config="item.content.config" :customStyle="item.content.customStyle" :datas="item.content.datas" :group="item.content.group"></customseven>
                          </div>
                        </div>
                      </div>
                      ...
                      <div class="component-compile-form-box">
                        ...
                        <customsevenform :key="active.no" v-if="active.id == 12" :config="active.content.config" :customStyle="active.content.customStyle" :datas="active.content.datas" :group="active.content.group"></customsevenform>
                      </div>
                    </div>
                      <!-- <div class="save-page">
                        <el-button type="primary" v-on:click="savePage">保存</el-button>
                        <el-button type="danger" @click="closeCao">取消</el-button>
                      </div> -->
                    </div>
                    <!--中心 over-->
                </div>
...
<script>
  ...
  import customseven from "./seven/seven.vue"
  import customsevenform from "./seven/sevenform.vue"
  ...
  function joinModule(items, parentId, level) {
  let array = [];
  for(let i in items) {
    if(items[i].moduleLevel == level && items[i].parentId == parentId) {
      let obj = {}
      obj.id = items[i].id;
      obj.label = items[i].moduleName
      obj.value = JSON.stringify({
        type: items[i].moduleType,
        path: items[i].modulePath,
        key: items[i].moduleKey,
        openType: items[i].openType
      })
      array.push(obj);
    }
  }
  for(let i in array) {
    let tempArray = joinModule(items, array[i].id, level + 1);
  ...
  export default {
    ...
    created() {
    let self = this;
    UTIL.vueRequest(self, "/miniprogram/wx/module/get.do", "post", "json", self.$qs.stringify({parentId: -1}), "application/x-www-form-urlencoded", function(data) {
    let items = data.data;
    Vue.prototype.$global.modules = joinModule(items, 0, 1);
    ...
    mounted() {
      ...
      promises.then(function() {
        UTIL.vueRequest(self, "/miniprogram/wx/shop/get.do", "post", "json", self.$qs.stringify({}),
          "application/x-www-form-urlencoded",
          function(data) {
            self.shop = data.data.id;
            self.page = data.data.pages.indexPage.id;
            self.pageType = data.data.pages.indexPage.pageType;

            self.$set(self.pageCustomAttribute, "pageTitle", data.data.shopName);
            self.$set(self.pageCustomAttribute, "backgroundColor", data.data.shopBackgroundColor);
            self.$set(self.pageCustomAttribute, "backgroundImage", data.data.shopBackgroundImage);

            let c = data.data.pages.indexPage.wxUserShopPageComponents;
            for(let i in c) {
              let obj = {};
              obj.no = (Math.random() * 10000000000000).toString();
              obj.id = c[i].componentId;
              obj.orderNum = c[i].orderNum;
              obj.content = JSON.parse(c[i].content);
    ...
    methods: {
      ...
      savePage() {
        ...
        shop.shopName = this.pageCustomAttribute.pageTitle;
        shop.shopBackgroundColor = this.pageCustomAttribute.backgroundColor;
        shop.shopBackgroundImage = this.pageCustomAttribute.backgroundImage;

        shop.pages = {};
        shop.pages.indexPage = {};
        shop.pages.indexPage.id = this.page;
        shop.pages.indexPage.shopId = this.shop;
        shop.pages.indexPage.pageType = this.pageType;
        shop.pages.indexPage.wxUserShopPageComponents = objs;
        UTIL.vueRequest(self, "/miniprogram/wx/shop/put.do", "post", "json", self.$qs.stringify({content: JSON.stringify(shop)}),
          "application/x-www-form-urlencoded",
          function(data) {
            console.log(data);
            if(data.status == 200){
